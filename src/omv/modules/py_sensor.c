...
static mp_obj_t py_sensor_set_auto_gain(uint n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    float gain_db = NAN;
    if (args[ARG_gain_db].u_obj != mp_const_none) {
        gain_db = mp_obj_get_float(args[ARG_gain_db].u_obj);
    }

    float gain_db_ceiling = NAN;
    if (args[ARG_gain_db_ceiling].u_obj != mp_const_none) {
        gain_db_ceiling = mp_obj_get_float(args[ARG_gain_db_ceiling].u_obj);
    }
