...
mp_obj_t py_tof_draw_depth(uint n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    float min = FLT_MAX;
    float max = -FLT_MAX;
    py_helper_arg_to_range(args[ARG_scale].u_obj, &min, &max, ir_array, src_img.w * src_img.h);
    ...
    tof_fill_image_float_obj(&src_img, ir_array, min, max);
...
mp_obj_t py_tof_snapshot(uint n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    float min = FLT_MAX;
    float max = -FLT_MAX;
    py_helper_arg_to_range(args[ARG_scale].u_obj, &min, &max, NULL, 0);
